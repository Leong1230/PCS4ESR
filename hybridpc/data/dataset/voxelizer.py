...
import collections
import numpy as np
from hybridpc.data.dataset.voxelization_utils import sparse_quantize
...
class Voxelizer:
    ...
    def voxelize(self, coords, feats, labels, center=None, link=None, return_ind=False):
        ...
        # transformed_coords = coords @ rigid_transformation[:3, :3].T
        ...
        return coords_aug.astype(np.float32), feats.astype(np.float32), labels.astype(np.float32), np.array(inds_reconstruct)
